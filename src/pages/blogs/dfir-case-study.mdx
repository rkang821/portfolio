...
**Incident**: In 2022, a major telecommunications company experienced a significant security breach due to a sophisticated social engineering attack. The attackers used SMS phishing, also known as “<a href="https://www.fcc.gov/avoid-temptation-smishing-scams" target="_blank" rel="noopener noreferrer">smishing</a>,” to deceive employees into revealing their credentials. This attack resulted in unauthorized access to sensitive information across a limited number of the company’s customer accounts.

**Detection and Initial Response**: Employees received text messages that appeared to be from the company’s IT department, claiming their passwords had expired or their schedules had changed, and prompting them to log in via malicious phishing link. This fraudulent site mimicked the company’s login page, capturing the credentials of unsuspecting victims who fell for the phish. Upon discovery, the security team promptly revoked access to the compromised employee accounts and engaged our team to assist with the investigation.

**Investigation and Analysis**: The investigation revealed that the attackers had meticulously linked employees’ phone numbers to their names to send targeted phishing messages, possibly using data from previous breaches. The forensic analysis focused on identifying the scope of the breach across the company's customer accounts, as well as tracing the origins of the phishing messages. The company worked with major US carriers and the hosting providers of the malicious URLs to shut down the attackers’ operations.
...
- **SMS Phishing Tactics**:
  ...
  - The malicious URLs directed employees to fake login pages mimicking the company's original login page, further convincing employees to enter their credentials.
...
- Okta Multi-Factor Authentication (MFA) logs
- CrowdStrike Falcon Insight XDR SIEM
...
**Other major players in the game**: Several cyber threat groups leverage supply chain attacks as a primary method of compromise, infiltrating software and service providers to gain access to their customers and downstream targets. These attacks exploit the trust and access granted to third-party vendors, making them highly effective and difficult to detect.

1. **Oktapus**: This group conducts mass phishing campaigns targeting employees at companies using Okta, leveraging supply chain access to infiltrate organizations and exfiltrate data.
2. **APT29 (Cozy Bear)**: This group is linked to Russian intelligence and is known for its sophisticated supply chain attacks, including the infamous <a href="https://www.solarwinds.com/sa-overview/securityadvisory/faq"  target="_blank" rel="noopener noreferrer">SolarWinds attack</a>. They infiltrate software supply chains to gain access to numerous downstream targets.
3. **APT41 (Winnti)**: A Chinese cyber espionage group, APT41 conducts supply chain attacks to deliver malware to specific targets. They have been linked to multiple high-profile incidents affecting various industries globally.
4. **Lazarus Group**: Associated with North Korea, Lazarus uses supply chain attacks as part of their broader cyber warfare strategy. They target software updates and distribution mechanisms to infect end users. 
