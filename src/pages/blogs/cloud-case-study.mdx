...
<blockquote>The following case studies are purely hypothetical and do not reflect any narratives from actual client engagements.</blockquote>
...
**Incident**: In this high-impact security incident, a threat actor compromised a company's Azure environment and illicitly spun up thousands of Virtual Machines (VMs) for cryptomining activities. This unauthorized activity led to substantial resource consumption and increased operational costs.
...
**Investigation and Analysis**: A detailed investigation was launched using Azure Security Center and Azure Log Analytics to trace the origin and scope of the breach. By examining system logs and network traffic data, we determined the entry point to be a set of compromised user credentials allowing for the attacker to access Azure's management portal. See below for a dummy sample Azure Log Analytics record reflecting similar activity:
